...
export const InspectionHistory: React.FC = () => {
  const { inspections, batchSessions } = useInspections();
  const [searchParams, setSearchParams] = useSearchParams();
  const navigate = useNavigate();

  const queryParam = searchParams.get('q') || '';
  const [searchQuery, setSearchQuery] = useState(queryParam);
  const [inspectionTypeFilter, setInspectionTypeFilter] = useState<'ALL' | 'SINGLE' | 'BATCH'>('ALL');
  ...
  const filteredBatchSessions = batchSessions.filter(s => {
    const matchesSearch = searchQuery.trim() === '' ||
      ...
      (s.sessionName && s.sessionName.toLowerCase().includes(searchQuery.toLowerCase())) ||
      s.products.some(p => p.productName.toLowerCase().includes(searchQuery.toLowerCase()) || p.brand.toLowerCase().includes(searchQuery.toLowerCase()));

    const matchesStatus = selectedStatus === 'ALL' || 
      (selectedStatus === 'Verified' && s.status === 'Complete') ||
      (selectedStatus === 'Review Required' && s.reviewRequiredCount > 0) ||
      (selectedStatus === 'Violation' && s.products.some(p => p.priority === 'High Priority'));

    return matchesSearch && matchesStatus;
  });

  const totalCount = 
    inspectionTypeFilter === 'SINGLE' ? filtered.length :
    inspectionTypeFilter === 'BATCH' ? filteredBatchSessions.length :
    filtered.length + filteredBatchSessions.length;
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-subtle flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <Badge variant="blue">{totalCount} Records</Badge>
          </div>
          <p className="text-xs text-slate-500 mt-1">
            Tamper-proof audit ledger of single package forensic dockets and multi-product batch shelf inspection sessions
          </p>
        </div>
      ...
      {/* Advanced Filter Matrix with Type Switcher */}
      <Card>
        <CardContent className="p-4 space-y-3">
          {/* Top Type Switcher Tabs */}
          <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
            <span className="text-xs font-bold text-slate-600 mr-2">Inspection Category:</span>
            {[
              { id: 'ALL', label: 'All Records', count: inspections.length + batchSessions.length },
              { id: 'SINGLE', label: 'Single Product Dockets', count: inspections.length },
              { id: 'BATCH', label: 'Batch / Shelf Sessions', count: batchSessions.length },
            ].map((tab) => (
              <button
                key={tab.id}
                type="button"
                onClick={() => setInspectionTypeFilter(tab.id as any)}
                className={cn(
                  "px-3 py-1.5 rounded-lg text-xs font-bold transition flex items-center gap-1.5",
                  inspectionTypeFilter === tab.id
                    ? "bg-blue-50 text-blue-900 border border-blue-200 shadow-xs"
                    : "text-slate-600 hover:bg-slate-100 border border-transparent"
                )}
              >
                <span>{tab.label}</span>
                <span className="text-[10px] font-mono font-bold bg-white/80 px-1.5 py-0.2 rounded border border-slate-200">
                  {tab.count}
                </span>
              </button>
            ))}
          ...
          <div className="pt-2 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
            ...
            <span className="text-slate-500 font-mono text-[11px]">
              Showing {totalCount} matching inspection items
            </span>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <div className="overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>Inspection ID / Type</TableHead>
                <TableHead>Product / Session</TableHead>
                ...
                <TableHead>Violations / Scope</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {/* Batch Sessions Rows */}
              {(inspectionTypeFilter === 'ALL' || inspectionTypeFilter === 'BATCH') && 
                filteredBatchSessions.map((session) => (
                  <TableRow
                    key={session.id}
                    onClick={() => navigate(`/inspection/session/${session.id}`)}
                    className="cursor-pointer bg-indigo-50/20 hover:bg-indigo-50/60 border-l-4 border-l-indigo-600"
                  >
                    <TableCell>
                      <Link
                        to={`/inspection/session/${session.id}`}
                        onClick={(e) => e.stopPropagation()}
                        className="font-mono font-bold text-indigo-700 hover:underline flex items-center gap-1.5 text-xs"
                      >
                        <Layers className="w-3.5 h-3.5 text-indigo-600 shrink-0" />
                        <span>{session.id}</span>
                        <ArrowUpRight className="w-3 h-3 opacity-60" />
                      </Link>
                      <span className="text-[10px] font-bold text-indigo-800 bg-indigo-100 px-1.5 py-0.2 rounded font-mono block mt-0.5 w-fit">
                        Batch Inspection
                      </span>
                    </TableCell>

                    <TableCell className="max-w-[200px]">
                      <div className="font-bold text-slate-900 truncate text-xs" title={session.sessionName || session.locationName}>
                        {session.sessionName || 'Batch Shelf Inspection'}
                      </div>
                      <div className="text-[11px] text-indigo-700 font-medium">
                        {session.totalProducts} products detected • {session.reviewRequiredCount} review req.
                      </div>
                    </TableCell>

                    <TableCell>
                      <span className="font-bold text-slate-900 text-xs">Multi-Brand ({session.products.length})</span>
                    </TableCell>

                    <TableCell className="text-xs text-indigo-800 font-medium">
                      Batch / Shelf Scan
                    </TableCell>
                    ...
                    <TableCell className="text-xs text-slate-600 max-w-[130px] truncate" title={session.locationName}>
                      {session.locationName}
                    </TableCell>

                    <TableCell>
                      <span className="font-mono font-bold text-xs text-emerald-700">
                        {session.completedCount}/{session.totalProducts} OK
                      </span>
                    </TableCell>
                    ...
                    <TableCell className="text-xs text-slate-700 whitespace-nowrap">
                      {session.inspectorName.split(' ')[1] || session.inspectorName} ({session.inspectorBadge.split('-')[3] || 'KA'})
                    </TableCell>

                    <TableCell>
                      <div className="flex flex-col gap-0.5">
                        <span className="font-mono text-xs font-bold text-indigo-800 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-200 w-fit">
                          {session.totalProducts} Products
                        </span>
                        {session.reviewRequiredCount > 0 && (
                          <span className="font-mono text-[10px] text-amber-700 font-bold">
                            {session.reviewRequiredCount} Review Req.
                          </span>
                        )}
                      </div>
                    </TableCell>
              ...
              {/* Single Product Rows */}
              {(inspectionTypeFilter === 'ALL' || inspectionTypeFilter === 'SINGLE') && 
                filtered.map((insp) => {
